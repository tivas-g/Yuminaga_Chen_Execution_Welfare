...
def fetch_trade_by_tx(tx_hash):
    ...
    if record2write['recipient_address'] =='0x0000000000000000000000000000000000000000':
        record2write['recipient_address'] = record2write['maker_address'] 
    print(parseCalldata.decode4inputassetaddress(calldata))
    print(parseCalldata.decode4outputassetaddress(calldata))
    print(tx['hash'])
    # if record2write['input_asset']  in [PEPE,WETH] and record2write['output_asset']  in [PEPE,WETH]:
        #has usdc and weth, proceed to fetch token data & reserve data & block time data
    recipient_address = record2write['recipient_address']
...
def get_empty_txs():
    ...
    cursor = conn.cursor()
    sql = '''SELECT tx_hash FROM txs WHERE venue = "1INCH" and output_amount is NULL'''
    cursor.execute(sql)
...
def main():
    ...
    # startblock = get_max_block_number(db_path) - 1
    # #startblock = 18000021
